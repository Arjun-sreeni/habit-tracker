...
def create_habit(habit_data: HabitCreate, user: User =  Depends(get_current_user), db: Session = Depends(get_db)):
    user_id = user.id
...
def get_habit_by_id(habit_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    user_id = user.id
...
def update_habit(habit_id: int, update_date: HabitUpdate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    user_id = user.id
