...
def test_duplicate_email(client):
    data = {"email": "test@gmail.com", "password": "secret1234"}
...
def test_login_succed(client):
    ...
    register_res = client.post("/auth/register", json=data)

    login_res = client.post("/auth/login", json=data)
...
def test_login_wrong_password(client):
    data = {"email": "test@gmail.com", "password": "secret123"}
    wrong_password_data = {"email": "test@gmail.com", "password": "secret12345"}

    client.post("/auth/register", json=data)

    res = client.post("/auth/login", json=wrong_password_data)
...
def test_protected_route(client):
    ...
    login_res = client.get("/auth/me", headers={"Authorization": f"Bearer {token}"})
